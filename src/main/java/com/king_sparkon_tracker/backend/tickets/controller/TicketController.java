...
public class TicketController {
    ...
    public TicketEventResponse createEvent(@Valid @RequestBody CreateEventRequest request, Principal principal) {
        return ticketManagementService.createEvent(request, actorUsername(principal));
    }

    @PatchMapping("/events/{eventId}")
    public TicketEventResponse updateEvent(
            @PathVariable String eventId,
            @Valid @RequestBody UpdateEventRequest request,
            Principal principal) {
        return ticketManagementService.updateEvent(eventId, request, actorUsername(principal));
    }

    @PostMapping("/purchase")
    @ResponseStatus(HttpStatus.CREATED)
    @IdempotentRequest(scope = "ticket-purchase")
    public TicketPurchaseResponse purchaseTickets(@Valid @RequestBody PurchaseTicketsRequest request, Principal principal) {
        return ticketManagementService.purchaseTickets(request, actorUsername(principal));
    }

    @GetMapping("/my-tickets")
    public List<UserTicketResponse> getMyTickets(Principal principal) {
        com.king_sparkon_tracker.backend.model.TrackerUser actor = ticketManagementService.currentActor(actorUsername(principal));
        return ticketManagementService.getMyTickets(String.valueOf(actor.getId()));
    }
    ...
    public GateVerificationResponse verifyTicketByQr(@Valid @RequestBody GateVerificationRequest request, Principal principal) {
        return ticketIdentityService.verifyByQr(request, actorUsername(principal));
    }
    ...
    public GateVerificationResponse verifyTicketByReference(@Valid @RequestBody GateVerificationRequest request, Principal principal) {
        return ticketIdentityService.verifyByReference(request, actorUsername(principal));
    }

    @GetMapping("/owner/dashboard")
    public OwnerTicketDashboardResponse getOwnerDashboard(Principal principal) {
        return ticketManagementService.getOwnerDashboard(actorUsername(principal));
    }

    @GetMapping("/owner/events")
    public List<TicketEventResponse> getOwnerEvents(Principal principal) {
        return ticketManagementService.getOwnerEvents(actorUsername(principal));
    }

    @PostMapping("/owner/withdrawals")
    @ResponseStatus(HttpStatus.CREATED)
    @IdempotentRequest(scope = "ticket-withdrawal")
    public TicketWithdrawalResponse requestWithdrawal(@Valid @RequestBody TicketWithdrawalRequest request, Principal principal) {
        return ticketManagementService.requestWithdrawal(request, actorUsername(principal));
    }

    @GetMapping("/owner/withdrawals")
    public List<TicketWithdrawalResponse> getWithdrawals(Principal principal) {
        return ticketManagementService.getWithdrawals(actorUsername(principal));
    }
