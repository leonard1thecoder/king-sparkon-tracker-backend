...
require_absent_from_active_gke_cd_files() {
  local blocked="$1"
  local label="$2"
  local matches

  matches=$(grep -RInF "$blocked" k8s .github/workflows/gke-cd.yml 2>/dev/null || true)

  if [[ -n "$matches" ]]; then
    while IFS= read -r match; do
      echo "::error::$match"
    done <<< "$matches"
    fail "$label must not be required in active GKE/CD files; use Workload Identity"
  else
    pass "$label not found in active GKE/CD files"
  fi
}
...
blocked_google_credential_path_var="GOOGLE_APPLICATION""_CREDENTIALS"
require_absent_from_active_gke_cd_files "$blocked_google_credential_path_var" "$blocked_google_credential_path_var"
